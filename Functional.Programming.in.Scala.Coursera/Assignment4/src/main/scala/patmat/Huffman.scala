/**
  * Screw this assignment. For reasons unknown, my implementation caused a stack overflow.
  * I've copied some of the function implementations to test if the issue was mine - yup, that's cheating, but
  * I don't want the certificate (and, in this case, nor would I deserve it :).
  */
package patmat
...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = chars match {
      ...
      // We could use this if we had a string, which returns a Map[Char, Int]
      // chars.groupBy(_).mapValues(_.size)
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    // We can't pattern match tuples here, so we refer to it as l1 and l2
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    // Transform the list into a map, so we can find the values by using its native methods
